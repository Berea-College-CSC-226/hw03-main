# Fully Functional Gitty Psychedelic Robotic Turtles

## Instructions

1. In the top right of Pycharm, change the display of this file to 
   `Editor and Preview` mode, so you can see the code (markdown) and the rendered output. 

![Screenshot of "Editor and Preview" mode](split_mode_markdown.png)

In PyCharm, the next line should appear red in the `Preview` mode on the right:

**_<span style="color:red">
    VERY IMPORTANT: Make a copy of this file. DO NOT EDIT IT DIRECTLY!
</span>_**

2. Make a copy of this file by selecting the file and hitting CTRL+C.
3. Paste your copy into the `Answers` folder.
4. Rename the file to `hw03_username.md` replacing `username` with your username.
5. Replace each `Replace this text with your answer` with your answer to the question above it.

_Return to the Google Doc to continue this assignment._

---

## SECTION 1

1.a. Using the [RGB Color Wheel tool](https://colorspire.com/rgb-color-wheel/), create the color purple. What are the R,
G, and B values?

```
    rgb(126, 27, 218)
```

1.b. Using the [RGB Color Wheel tool](https://colorspire.com/rgb-color-wheel/), create the color brown. 
     What are the R, G, and B values? 

```
    rgb(95, 61, 10)
```

1.c. Using the [RGB Color Wheel tool](https://colorspire.com/rgb-color-wheel/), create the color xanadu. 
     What are the R, G, and B values?

```
    rgb(122, 145, 106)
```

_Return to the Google Doc to continue this assignment._

---

### SECTION 2

2.a. Explore the turtle library documentation and find the description for the
`forward()` method. What alternate command can be used to move the turtle forward, besides the `turtle.forward()`
command you are used to using?

```
    We can use turtle.fd(distance) to move the turtle forward. The fd is short for forward.
```

2.b. Which command from the turtle library can be used to print the turtle's current location?

```
    turtle.position(), turtle.pos() these two commands can be used to print the turtle's current location.
```

2.c. How do you set the turtle's speed to maximum speed?

```
    turtle.speed(10) and if you want no animation you can use turtle.speed(0)
```

2.d. How would you change the turtle's color to xanadu?

```
    turtle.color(122, 145, 106)
```

2.e. How would you fill a shape with the color xanadu?

```
    turtle.fillcolor(122, 145, 106)
```

_Return to the Google Doc to continue this assignment._

---

## SECTION 3

3.a. What does **cloning** the repo mean?

```
    Copying all the codes from the repo.
```

- What is the **repository**? Where does it exist (on your local machine or in Github)?

```
    It exists on Github.
```

- What is a **commit**? Why does it need a commit message?

```
    Commit is something you do if you want to save your progress. And with the message you can know later what you changed in that commit.
```

- What does it mean to **push** your code? Where is your code being pushed _to_ and _from_?

```
    It means to send the commits to github where it can be stored. From your computer to the cloud.
```

_Return to the Google Doc to continue this assignment._

---

## SECTION 4

## Pull Main Into Your Local

4.a. Why do you think it is important to pull before you push?

```
    It is important to get the latest code on GITHUB. So you get updated what your teammates did.
```

4.b. How many branches are in the repository? Click the link to look at the branches. Do you see yours? Do you see any
others?

```
    There are 50 branches on the repository at the moment. I do not see mine bcs after I created the branch I did not push after I finish sectoin 4 I will push.
```


4.c. Compare your branch and the main branch by clicking on each. Are they different?

```
    They are different bcs I added something on my branch.
```


4.d. Go back to PyCharm, and go back to the Branches interface from before. Checkout the 
     main branch. Describe what happens to your file in the Project pane of PyCharm. Is it still 
     there? Did it change?

```
    When I go to main my codes are not there becuase i didnt commit push and merged my code with the main yet.
```

4.e. Now go back to your branch in PyCharm. Is your file back? Based on your observations here, describe how branching
is useful:

```
    Yeah my file is back. With branches if I ever do something wrong I can correct it from github. Also with branches you can seperate the main codes from your codes which is like a practice codes.
```

_Return to the Google Doc to continue this assignment._

---

## SECTION 5

A lot happened in this assignment, and often, you do things without fully understanding them. Your last task is to 
formulate a question and ask it. To do this, put your question into the [Slack channel](https://bereacs.slack.com/archives/C3QACGH8R) and the TAs and the 
instructor will answer them for everyone! Paste the link to your question in Slack here:

```
    https://bereacs.slack.com/archives/C3QACGH8R/p1789492219015869
```

---
