...
```
    you can use the turtle.backward() and insert a negative value
```
...
```
    cloning the repo is  creating a copy of the repository from github to your personal machine
```
...
```
    storage location for code, files. it exists on github and is copied to the local machine.
    needs to be manually updated(pull before push)
```
...
```
    a commit is a saved change in the code or files, a commit message is needed to clarify what was changed or added for 
    other programmers
```
...
```
    pushing your code is saving the changes to githubs servers so that collaborators can see and use the code.
```
