...
```
    Cloning means making a copy of the project from Github onto my own computer.
```
...
```
    A repository is the folder that holds all the code and it exists on both Github and 
    my computer when I clone it.
...
```
    A commit is like a saved snapshot of my code and the message explains what changes I made.
```
...
```
    Pushing sends my saved changes from my computer up to Github so others can see them.
```
...
```
    What is the difference between merging and pulling ? 
```
