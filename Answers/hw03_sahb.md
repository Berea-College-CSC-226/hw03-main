...
```
    I didn't find it in colorspire but I found it on google rgb(115, 134, 120)
```
...
```
    Cloning means making the copy of the main branch from github to our computer. 
```
...
```
    Repository is like a folder that stores all files and codes . It exist both in Github and our local computer. 
```
...
```
    Commit is like a snapshot of our code. A commit is needed because we track when and what changes we made. 
```
...
```
    ...
    Pushing code means sending committed changes from local computer to the remote repository on GitHub. 
    My code is pushed from my local machine to GitHub.
...
```
    If I accidentally made commits , what’s the safest way to delete them?
```
